...
class Transfer(nn.Module):
    ...
    def __init__(self, n_channel = 128, d_out = 1 , n_conv_layer = 10, n_fc_layer = 0):
        super().__init__()
        ...
        for i in range(n_fc_layer):
            fc_list.append(nn.Linear(n_channel, n_channel))
        self.fc_list = nn.ModuleList(fc_list)
        self.fc_fin = nn.Linear(n_channel, d_out)
...
model = Transfer(n_channel = 128, d_out = 1, n_conv_layer = 3, n_fc_layer = 0)
